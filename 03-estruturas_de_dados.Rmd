...
O objetivo desse exemplo é mostrar uma aplicação de como as listas são usadas dentro R, faremos isso mostrando que ao efetuar uma regressão no R os resultados são armazenados em listas. Para tal vamos utilizar o dataset *gapminder* (disponível no pacote gapminder) que é um excerto dos dados de expectativa de vida, PIB per capita e população por país. Vamos construir um modelo onde a expectativa de vida depende do PIB percapita e do tamanho da população.  
```
