...
matrix_2 <- matrix(data = 1:6, ncol = 3)
```

#### Multiplicação e Soma {-}

De maneira geral podemos nos deparar com dois casos: (i) queremos somar/multiplicar um número com/por uma matriz ou (ii) queremos somar/multiplicar duas matrizes. 

Quando nos encontramos no primeiro caso basta usar os operadores apresentados na seção **colocar aqui a referencia para a seção R como calculadora** e a operação é aplicada em cada elemento da matrix, como feito no chunk que segue
```{r}
3+matrix_1
3*matrix_2
```
